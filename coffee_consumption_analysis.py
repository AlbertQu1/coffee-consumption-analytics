...
columnas_tazas = ['espresso', 'quad', '6oz',
                  '8oz', '10oz', '12oz', '14oz', '16oz', '18oz']
...
ALERTA_INVENTARIO_DIAS = 2    # alerta por días restantes
...
df[columnas_tazas] = df[columnas_tazas].apply(
...
if col_total_tazas_decl:
    ...
    print(
        f"✅ La columna '{col_total_tazas_decl}' ha sido actualizada con la suma por tamaños en cada fila.")


# ===================== CÁLCULOS BASE =====================
precio_promedio_cafeteria = sum(
    precio_cafeteria.values()) / len(precio_cafeteria)
...
# Costo por gramo
costo_por_gramo = (costo_total_cafe /
                   gramos_comprados) if gramos_comprados > 0 else np.inf

# Costo por taza por tipo y costo promedio ponderado por tu mezcla real
costo_taza_tipo = {
    t: (gramos_por_taza[t] *
        costo_por_gramo) if np.isfinite(costo_por_gramo) else np.inf
    for t in columnas_tazas
}

if total_tazas > 0 and np.isfinite(costo_por_gramo):
    costo_promedio_taza = sum(
        costo_taza_tipo[t] * total_tazas_tipo[t] for t in columnas_tazas) / total_tazas
else:
    costo_promedio_taza = np.inf

# Ahorro y recuperación vs promedio de cafeterías
ahorro_por_taza = precio_promedio_cafeteria - \
    costo_promedio_taza if np.isfinite(costo_promedio_taza) else -np.inf
tazas_para_recuperar = (
    costo_cafetera / ahorro_por_taza) if (ahorro_por_taza > 0) else np.inf
ahorro_total = (max(0, ahorro_por_taza) * total_tazas) if (total_tazas >
                                                           0 and np.isfinite(ahorro_por_taza)) else 0.0
...
if dias_consumo and (dias_consumo > 0) and (gramos_consumidos > 0):
    ...
    ahorro_diario_real = ahorro_por_taza * \
        tazas_por_dia_real if ahorro_por_taza > 0 else 0
...
for nombre, precio_ext in precio_cafeteria.items():
    ahorro_taza_i = (
        precio_ext - costo_promedio_taza) if np.isfinite(costo_promedio_taza) else -np.inf
    tazas_rec_i = (costo_cafetera /
                   ahorro_taza_i) if (ahorro_taza_i > 0) else np.inf
    comparativa.append({
        'Cafetería': nombre,
...
comparativa_df = pd.DataFrame(comparativa).sort_values(
    'Precio externo (MXN)', ascending=False).reset_index(drop=True)

# ===================== REPORTE =====================
print('\n☕ RESUMEN PERSONAL DE CONSUMO Y AHORRO (con gramaje por tamaño)')
print(
    f'• Precio promedio en cafeterías externas: {fmt_mon(precio_promedio_cafeteria)}')
if np.isfinite(costo_promedio_taza):
    print(
        f'• Costo por gramo estimado en casa:       {fmt_mon(costo_por_gramo)} /g')
    print(
        f'• Costo promedio por taza en casa:        {fmt_mon(costo_promedio_taza)}')
else:
    print('• Costo por gramo/taza en casa:           N/D (faltan datos de gramos o tazas)')

print(
    f'• Café comprado:                           {fmt_num(cafe_comprado_kg)} kg')
print(
...
if (ahorro_por_taza > 0) and (total_tazas > 0) and np.isfinite(costo_promedio_taza):
    print(
        f'• Ahorro por taza (vs. promedio):          {fmt_mon(ahorro_por_taza)}')
    print(
        f'• Tazas para recuperar la cafetera:        {fmt_num(tazas_para_recuperar, 0)}')
    print(
        f'• Ahorro total acumulado:                  {fmt_mon(ahorro_total)}')
elif total_tazas == 0:
...
if inventario_estimado_kg <= ALERTA_INVENTARIO_KG:
    print(
        f'⚠️  Alerta: inventario bajo (≤ {ALERTA_INVENTARIO_KG} kg). Considera comprar café pronto.')
if (dias_restantes is not None) and np.isfinite(dias_restantes) and (dias_restantes <= ALERTA_INVENTARIO_DIAS):
    print(
        f'⚠️  Alerta: quedan ~{dias_restantes:.1f} días de inventario. ETA agotamiento: {fecha_agotamiento.date()}.')
...
col_cafeteria_csv = coalesce_col(
    df, ['Cafeteria', 'cafeteria', 'Proveedor', 'Tienda'])
